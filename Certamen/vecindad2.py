...
class Config:
    ...
    tamano = 500
    propagacion = True
    prop_transito = 0.00001
    # no traspase a las otras 
    #prop_transito = 0.0001   # Proporción de individuos que pueden transitar a otra celda en el caso de propagación
        ...
        for i, capa in enumerate(capas):
            for y in range(ALTO_CAPA):
                for x in range(ANCHO_CAPA):
                    ...
                    color = "gray" if valor_celda == SUSCEPTIBLE else "yellow" if valor_celda == EXPUESTO else "red" if valor_celda == INFECTADO else "purple" if valor_celda == RECUPERADO else "gray"
            ...
            for y in range(ALTO_CAPA):
                for x in range(ANCHO_CAPA):
                    if capas[i][y][x] == SUSCEPTIBLE:
                        ...
                        if CONFIG.propagacion and random.random() < CONFIG.prop_transito:
                            # Tránsito a una celda vecina
                            nueva_capa[y][x] = SUSCEPTIBLE
                            tiempo_expuesto[y][x] = 0
                            vecinos_disponibles = [(j, k) for j in [-1, 0, 1] for k in [-1, 0, 1]
                                                   if (j == 0 or k == 0)
                                                   and 0 <= x + j < ANCHO_CAPA and 0 <= y + k < ALTO_CAPA
                                                   and capas[i][y + k][x + j] == SUSCEPTIBLE]
                            if vecinos_disponibles:
                                j, k = random.choice(vecinos_disponibles)
                                nueva_capa[y + k][x + j] = INFECTADO
                        elif vecinos_i_misma_capa >= 1:
                        ...
                        tiempo_expuesto[y][x] += 1
                        if tiempo_expuesto[y][x] >= 5:  # Ajusta este valor según la duración de la infección
                            nueva_capa[y][x] = RECUPERADO
                    # Resto de la lógica SEIR
